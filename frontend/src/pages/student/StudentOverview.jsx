    ...
    return (
        <>
            ...
            <div className="space-y-10">
                ...
                {isImpersonating && (
                    <div className="bg-blue-600 text-white px-6 py-4 flex items-center justify-between shadow-2xl rounded-2xl">
                ...
                {/* Active class banner */}
                {activeClass && (
                    <div className="bg-blue-600 rounded-[3rem] p-10 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 blur-3xl rounded-full translate-x-1/2 -translate-y-1/2" />
                        <div className="flex items-center gap-10 relative z-10">
                            <div className="w-20 h-20 bg-white/20 backdrop-blur-xl rounded-[2rem] flex items-center justify-center text-4xl shadow-2xl">🏫</div>
                            <div>
                        ...
                        <button onClick={() => handleJoinClass(activeClass)} className="bg-white text-blue-700 hover:bg-blue-50 px-8 py-3 rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-lg shrink-0 relative z-10">
                            Join Class Now
                        </button>
                    </div>
                )}
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
                    <div>
                        ...
                        <p className="text-slate-500 font-medium text-sm flex items-center gap-2">
                            Welcome back, {user?.first_name} <span className="w-1.5 h-1.5 rounded-full bg-blue-600 animate-pulse inline-block" />
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                        { label: 'Wallet Balance', value: `₦${parseFloat(profile?.wallet_balance || 0).toLocaleString()}`, icon: IconWallet, colorClass: 'text-blue-600', bgClass: 'bg-blue-600/10', link: '/payment', action: 'Top up' },
                        ...
                        <motion.div key={i} variants={ITEM_VARIANTS} whileHover={{ y: -5 }} className="bg-white border border-slate-100 rounded-[2.5rem] p-8 overflow-hidden group shadow-[0_10px_40px_rgba(0,0,0,0.03)]">
                    ...
                    <div className="lg:col-span-2 space-y-10">
                        ...
                        {bookings.some(b => b.status === 'APPROVED' && !b.paid) && (
                            <div className="bg-blue-50 border border-blue-100 rounded-[3rem] p-10 shadow-sm">
                                <h3 className="text-xl font-display font-black text-slate-900 mb-6 flex items-center gap-4">
                                    <div className="w-1.5 h-6 bg-blue-600 rounded-full" /> Pending Payments
                                </h3>
                                <div className="space-y-4">
                                    {bookings.filter(b => b.status === 'APPROVED' && !b.paid).map(b => (
                                        <div key={b.id} className="bg-white border border-slate-100 p-6 rounded-2xl flex justify-between items-center shadow-sm">
                                            ...
                                            <button onClick={() => navigate(`/payment?booking_id=${b.id}`)} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Pay Now</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="bg-slate-50/50 border border-slate-100 rounded-[3rem] p-10">
                            <h3 className="text-xl font-display font-black text-slate-900 mb-8 flex items-center gap-4">
                                <div className="w-1.5 h-6 bg-blue-600 rounded-full" /> Registered Subjects & Class Links
                            </h3>
                            <div className="grid sm:grid-cols-2 gap-6">
                                {profile?.enrollments?.map((enr, i) => (
                                    <div key={`enr-${i}`} className="bg-white rounded-[2rem] p-6 border border-slate-100 hover:border-blue-600/30 transition-all shadow-sm">
                                        ...
                                        <div className="mt-6 pt-6 border-t border-slate-50 flex flex-col gap-4">
                                            <div className="grid grid-cols-2 gap-2 text-[9px] font-black text-slate-500 uppercase">
                                                ...
                                                <div className="flex items-center gap-1.5 bg-slate-50 p-2 rounded-lg">
                                                    <IconClock size={12} className="text-blue-600" /> {enr.hours_per_week || 0} Hrs / Session
                                                </div>
                                            </div>
                                            ...
                                            <Link to="/student/classes" className="w-full text-blue-600 py-1 font-bold text-[9px] uppercase tracking-widest hover:underline text-center">
                        ...
                        <div className="space-y-6">
                            <h3 className="text-xl font-display font-black text-slate-900 flex items-center gap-4">
                                <div className="w-1.5 h-6 bg-blue-600 rounded-full" /> Upcoming Sessions
                            </h3>
                            <div className="space-y-4">
                                {classes.length > 0 ? classes.slice(0, 3).map((cls, i) => (
                                    <div key={i} className="bg-white border border-slate-100 rounded-[2rem] p-6 flex flex-col md:flex-row items-center gap-6 hover:border-blue-600/30 transition-all shadow-sm">
                                        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center text-2xl shadow-inner">🎓</div>
                        ...
                        {profile?.admission_letter_url && profile?.approval_status === 'APPROVED' && (
                            <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-[2.5rem] p-8 shadow-2xl shadow-indigo-500/20 relative overflow-hidden">
                                ...
                                <div className="relative z-10 flex flex-col items-center text-center">
                                    ...
                                    <a href={profile.admission_letter_url} download target="_blank" rel="noreferrer" className="w-full py-4 bg-white text-indigo-600 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-3">
                            ...
                            <div className="flex-1 p-8 md:p-12 relative max-h-[90vh] overflow-y-auto">
                                ...
                                <div className="space-y-8">
                                            ...
                                            {enrollData.schedule.map((slot, index) => (
                                                <div key={index} className="flex gap-3 items-end">
                                                    <select id={`schedule_day_${index}`} name={`schedule_day_${index}`}
                                                        className="flex-1 bg-slate-50 border border-slate-100 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600/30 outline-none"
                                                        value={slot.day}
                                                        onChange={e => {
                                                    ...
                                                    <input id={`schedule_time_${index}`} name={`schedule_time_${index}`} type="time"
                                                        className="w-32 bg-slate-50 border border-slate-100 rounded-2xl p-4 font-bold text-slate-900 focus:border-blue-600/30 outline-none"
                                                        value={slot.time}
                                                        onChange={e => {
                                                            const s = [...enrollData.schedule];
                                    ...
                                    {scheduleStatus.message && (
                                        <div className={`p-4 border rounded-2xl flex items-center gap-3 ${scheduleStatus.status === 'error' ? 'bg-red-50 border-red-100' : 'bg-blue-50 border-blue-100'}`}>
                                            {scheduleStatus.status === 'error'
                                                ...
                                                : <IconCheckCircle2 className="text-blue-600" size={16} />}
                                            <p className={`text-[10px] font-black uppercase tracking-widest ${scheduleStatus.status === 'error' ? 'text-red-500' : 'text-blue-600'}`}>
                                                {scheduleStatus.message}
                                            </p>
                                        </div>
                                    )}
                                    ...
                                    <button
                                        ...
                                        className="w-full bg-blue-600 hover:bg-blue-700 text-white py-6 rounded-3xl font-black uppercase text-xs tracking-[0.3em] shadow-3xl active:scale-95 transition-all flex items-center justify-center gap-4 disabled:opacity-30 disabled:cursor-not-allowed"
                                    >
