...
const STATUS_MAP = {
    ...
    APPROVED: { label: 'Approved', icon: <CheckCircle2 size={13} />, cls: 'bg-blue-50 text-blue-600 border-blue-200' },
    ...
    if (!cls) {
        return (
            <div className="py-32 text-center">
                ...
                <button onClick={() => navigate('/student/classes')} className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-blue-700 transition-all">
                    ← Back to Classes
                </button>
            </div>
        );
        ...
        <>
            ...
            <div className="max-w-2xl space-y-6">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 bg-slate-50 text-slate-600 text-[10px] font-semibold uppercase tracking-widest">
                        {cls.type || 'Regular'}
                    </span>
                    {cls.is_live && (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-red-600 text-white text-[10px] font-black uppercase tracking-widest animate-pulse">
                            ● Live Now
                        </span>
                    )}
                </div>
                ...
                <div className="bg-white border border-slate-100 rounded-[2rem] p-8 space-y-5 shadow-sm">
                    <Row icon={<Calendar size={16} className="text-blue-600" />} label="Date">
                ...
                {(meetingLink || cls.whiteboard_link) && (
                    <div className="bg-white border border-slate-100 rounded-[2rem] p-8 space-y-4 shadow-sm">
                        ...
                        {meetingLink && (
                            <a
                                ...
                                className="flex items-center gap-3 px-5 py-3.5 bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-700 rounded-2xl text-sm font-semibold transition-all group"
                            >
    ...
    return (
        <div className="flex items-start gap-4">
            ...
            <div>
                ...
                <p className="text-slate-800 font-semibold text-sm">{children}</p>
            </div>
        </div>
    );
}
