        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-900">
                <div className="flex flex-col items-center gap-6">
                    ...
                    <p className="text-emerald-500 font-bold uppercase tracking-[0.3em] text-xs">Accessing Gateway...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="container pt-32 pb-20 px-4 relative z-10">
                <div className="max-w-4xl mx-auto grid lg:grid-cols-5 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-slate-900/50 backdrop-blur-xl border border-white/5 rounded-card-lg p-5 md:p-8 shadow-2xl">
                            <h2 className="text-[11px] font-semibold uppercase tracking-wide text-emerald-500 mb-6 flex items-center gap-2">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-3 pt-4 border-t border-white/5">
                                    ...
                                    <div className="flex justify-between items-center text-xs">
                                        ...
                                        <span className="text-lg font-bold text-emerald-500">₦{parseFloat(loadAmount || 0).toLocaleString()}</span>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="bg-emerald-600/5 backdrop-blur-md border border-emerald-500/10 rounded-card p-6">
                            <h4 className="text-[11px] font-semibold uppercase tracking-wide text-emerald-500 mb-4 flex items-center gap-2">
                                🛡️ Secure Gateway
                            </h4>
                    ...
                    <div className="lg:col-span-3">
                        <div className="bg-slate-900 border border-white/10 rounded-card-lg shadow-2xl p-10 overflow-hidden relative group">
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="space-y-4">
                                    <label className="block text-[11px] font-semibold uppercase tracking-wide text-slate-500 ml-1">Input Top-up Amount</label>
                                    <div className="relative">
                                        <span className="absolute left-6 top-1/2 -translate-y-1/2 text-2xl font-bold text-emerald-500/40">₦</span>
                                        <input
                                            type="number"
                                            value={loadAmount}
                                            onChange={(e) => setLoadAmount(e.target.value)}
                                            readOnly={!!bookingId || profile?.payment_status === 'UNPAID'}
                                            className={`w-full bg-white/5 border-2 border-white/5 rounded-3xl p-6 pl-12 font-bold text-3xl text-white outline-none focus:border-emerald-500/30 focus:bg-white/10 transition-all ${bookingId || profile?.payment_status === 'UNPAID' ? 'opacity-50 cursor-not-allowed' : ''}`}
                                            placeholder="0.00"
                                        />
                                    </div>
                                        ...
                                        <div className="flex flex-wrap gap-2 pt-2">
                                            {['5000', '10000', '25000', '50000'].map(amt => (
                                                <button 
                                                    ...
                                                    className={`px-4 py-2 rounded-xl text-[11px] font-semibold uppercase tracking-wide transition-all border ${loadAmount === amt ? 'bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-white/5 border-white/10 text-slate-500 hover:border-white/20'}`}
                                                >
                                ...
                                {/* Method Selector */}
                                <div className="space-y-4">
                                    <label className="block text-[11px] font-semibold uppercase tracking-wide text-slate-500 ml-1">Choose Payment Method</label>
                                    <div className="grid gap-4">
                                        {methods.map(m => (
                                            <button
                                                key={m.id}
                                                onClick={() => setPaymentMethod(m.id)}
                                                className={`flex items-center gap-5 p-5 rounded-card border-2 transition-all group ${paymentMethod === m.id ? 'bg-emerald-500/10 border-emerald-500 shadow-xl shadow-emerald-500/10' : 'bg-white/5 border-white/5 hover:border-white/10'}`}
                                            >
                                                ...
                                                <div className="text-left flex-1">
                                                    <h4 className={`font-bold uppercase tracking-widest text-[11px] ${paymentMethod === m.id ? 'text-emerald-500' : 'text-white'}`}>{m.name}</h4>
                                                ...
                                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${paymentMethod === m.id ? 'border-emerald-500 bg-emerald-500' : 'border-white/10'}`}>
                                                    {paymentMethod === m.id && <span className="text-white text-[10px] font-bold">✓</span>}
                                                </div>
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <button
                                    ...
                                    className={`w-full group/btn relative py-6 rounded-card-lg font-bold uppercase tracking-[0.3em] text-xs shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-30 disabled:cursor-not-allowed overflow-hidden ${
                                        !paymentMethod 
