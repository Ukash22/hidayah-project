    ...
    return (
        <>
            ...
            <div className="space-y-6 pb-20">
                ...
                {/* Revenue Chart */}
                <Suspense fallback={<div className="h-[380px] bg-slate-50 dark:bg-slate-800/60 rounded-card-lg animate-pulse" />}>
                    <AdminRevenueChart
                ...
                {/* Payment History */}
                <div className="bg-white dark:bg-slate-900 rounded-card-lg shadow-xl shadow-slate-200/50 border border-slate-100 dark:border-slate-800 overflow-hidden">
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {!financials?.history || financials.history.length === 0 ? (
                                    ...
                                    <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-all group">
                                        ...
                                        <td className="py-4 px-6">
                                            <span className={`text-[9px] px-2 py-1 rounded-lg font-bold uppercase tracking-tighter ${p.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-700' : p.status === 'FAILED' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{p.status}</span>
                                        </td>
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {transactions.length === 0 ? (
                                    ...
                                    <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-all group">
                                        ...
                                        <td className="py-4 px-6">
                                            <div className={`text-sm font-bold ${(t.type?.includes('DEBIT') || t.type?.includes('WITHDRAWAL')) ? 'text-red-600' : 'text-emerald-600'}`}>
