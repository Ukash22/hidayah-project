...
export default function TutorRequests() {
    ...
    const fetchRequests = useCallback(async () => {
        if (!token) return;
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/classes/booking/approval/`, { headers: getAuthHeader() });
            ...
            <div className="flex bg-slate-100 p-1.5 rounded-2xl overflow-x-auto mb-8 w-fit">
                {['pending', 'approved', 'active'].map(sub => (
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeSubTab === sub ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-700'}`}
                    >
                    ...
                    <div key={req.id} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col gap-8 hover:border-blue-600/30 transition-all">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                            ...
                            <div className="flex flex-col items-end text-right">
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Total Fee</span>
                        ...
                        <div className="grid md:grid-cols-3 gap-6 pt-8 border-t border-slate-100">
                            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 block mb-3">Target Schedule</span>
                                <div className="space-y-2">
                                    {(() => {
                                        try {
                                            const sched = typeof req.schedule === 'string' ? JSON.parse(req.schedule) : req.schedule;
                                            if (!sched || !Array.isArray(sched)) return <span className="text-[11px] font-bold text-slate-400 italic">No schedule provided</span>;
                                            ...
                                            return <span className="text-[11px] font-bold text-slate-400 italic">Multiple Slots Requested</span>;
                                        }
                                    })()}
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 flex flex-col justify-center gap-2">
                                <div className="flex justify-between items-center">
                                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Hours/Week</span>
                                    <span className="text-sm font-black text-slate-900">{req.hours_per_week}h</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Start Date</span>
