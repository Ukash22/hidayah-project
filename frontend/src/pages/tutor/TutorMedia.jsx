    ...
    return (
        <>
            ...
            {tutorProfile && (tutorProfile.video_url || tutorProfile.recitation_url) && (
                <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm space-y-8 mb-10">
                    <h3 className="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em]">Currently Active Assets</h3>
                    <div className="grid md:grid-cols-2 gap-8">
                        {tutorProfile.video_url && (
                            <div className="space-y-4">
                                ...
                                {tutorProfile.video_type === 'youtube' ? (() => {
                                    ...
                                    return (
                                        <div className="aspect-video rounded-2xl overflow-hidden border border-slate-100 shadow-lg">
                                            <iframe
                                    ...
                                    <video
                                        ...
                                        controls className="w-full rounded-2xl border border-slate-100 shadow-lg"
                                    />
                                )}
                            </div>
                        )}
                        {tutorProfile.recitation_url && (
                            <div className="space-y-4">
                                <p className="text-[10px] font-black uppercase text-blue-600 tracking-widest ml-1">Recitation Audio</p>
                                <div className="bg-blue-50 p-8 rounded-2xl border border-blue-100 flex flex-col items-center gap-6">
                                    <div className="w-16 h-16 bg-blue-600/10 rounded-2xl flex items-center justify-center text-3xl border border-blue-100">🎙️</div>
                                    <audio
            ...
            <div className="grid md:grid-cols-2 gap-10">
                <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm space-y-8 hover:border-blue-600/30 transition-all">
                    ...
                    <label className="block">
                        <div className="border-2 border-dashed border-slate-200 rounded-3xl p-10 text-center cursor-pointer hover:bg-slate-50 hover:border-blue-600/30 transition-all">
                ...
                <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm space-y-8 hover:border-indigo-500/30 transition-all">
                    <div className="flex items-center gap-5">
                    ...
                    <label className="block">
                        <div className="border-2 border-dashed border-slate-200 rounded-3xl p-10 text-center cursor-pointer hover:bg-slate-50 hover:border-indigo-500/30 transition-all">
            ...
            {(mediaFiles.intro_video || mediaFiles.short_recitation) && (
                <div className="flex justify-center pt-8">
                    <button
                        ...
                        disabled={!!uploading}
                        className="bg-blue-600 text-white px-12 py-5 rounded-2xl font-black uppercase tracking-[0.3em] text-xs shadow-2xl shadow-blue-600/30 hover:scale-[1.05] active:scale-95 transition-all disabled:opacity-50 flex items-center gap-4"
                    >
                        {uploading ? (
            ...
            {uploadMsg && (
                <p className={`text-center font-black text-[10px] uppercase tracking-widest mt-6 ${uploadMsg.startsWith('✅') ? 'text-blue-600' : 'text-red-500'}`}>{uploadMsg}</p>
            )}
        </>
    );
}
