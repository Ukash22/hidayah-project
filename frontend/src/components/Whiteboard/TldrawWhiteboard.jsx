                ...
                if (file) {
                    ...
                    reader.onload = () => {
                        editor.createShapes([{
                            type: 'image',
                            x: 100, y: 100,
                            props: { src: reader.result, w: 300, h: 300 }
                        }]);
                    };
...
const CustomHeader = ({ editor, activeTab, setActiveTab, role, onPush, onDownload, activeStudentName, studentCount }) => {
    return (
                                ...
                                {Object.keys(studentThumbnails).length === 0 ? (
                                    ...
                                    Object.entries(studentThumbnails).map(([id, data]) => (
                                        <div key={id} className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden hover:shadow-2xl transition-all group border-b-4 border-b-emerald-500/50">
                                            ...
                                            <div className="h-64 bg-white p-6 relative flex items-center justify-center">
                                                {data.svg ? (
                                            ...
                                            <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3">
                                                <button 
                                                    ...
                                                    className="flex-1 py-3.5 bg-[#1e293b] text-white text-[11px] font-black uppercase tracking-widest rounded-2xl hover:bg-slate-700 transition-all shadow-xl shadow-slate-900/20 flex items-center justify-center gap-2"
                                                >
                                                    Coach Student ⚡
                                                </button>
                                                <button className="w-14 h-14 bg-white text-slate-600 rounded-2xl border border-slate-200 hover:bg-emerald-50 hover:text-emerald-500 transition-all flex items-center justify-center text-xl shadow-sm">
                                                    👍
                                                </button>
                                            </div>
                                        </div>
